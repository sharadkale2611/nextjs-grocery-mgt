...
export const stockLedgerApi = api.injectEndpoints({
  endpoints: (builder) => ({
    ...
    // =============================
    // GET LEDGER BY PRODUCT ID
    // =============================
    getStockLedgerByProductId:
      builder.query<StockLedger, number>({
        query: (productId) =>
          `${API_ROUTES.PRODUCT_STOCKS}/product/${productId}`,

        transformResponse:
          (res: ApiResponse<StockLedger>) =>
            res.data ,

        providesTags: (result, error, productId) => [{ type: "StockLedger", id: productId }],
      }),
...
export const {
  ...
  useGetStockLedgerByProductIdQuery,
